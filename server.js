...
const providers = {
  baidu: {
    ...
    async profile(token) {
      ...
      return { id: p.userid || p.openid || '', name: p.username || p.realname || 'Baidu user' };
    },
  ...
  yuque: {
    ...
    async profile(token) {
      ...
      const d = p.data || p;
      return { id: d.id || '', name: d.name || d.login || 'Yuque user', login: d.login || '' };
    },
  ...
  feishu: {
    ...
    async profile(token) {
      ...
      const d = p.data || p;
      return { id: d.open_id || d.user_id || '', name: d.name || d.en_name || 'Feishu user' };
    },
    token(data) { return data.access_token || data.data?.access_token; },
    refresh(data) { return data.refresh_token || data.data?.refresh_token || ''; },
    expires(data) { return Number(data.expires_in || data.data?.expires_in || 0); }
  ...
  dingtalk: {
    ...
    async profile(token) {
      ...
      return { id: p.openId || p.unionId || '', name: p.nick || p.name || 'DingTalk user' };
    },
...
function validateProvider(name) {
  ...
  if (!p) throw new Error('Unsupported OAuth provider');
  const missing = [];
  ...
  if (missing.length) {
    const err = new Error(`${p.label} OAuth is not configured: ${missing.join(', ')}`);
    err.status = 503;
    throw err;
  }
  return p;
...
app.get('/api/oauth/:provider/start', (req, res) => {
  try {
    ...
    if (!sessionId || sessionId.length > 200) return res.status(400).json({ error: 'A valid sessionId is required' });
    ...
    if (allowedOrigins.length && returnOrigin && !allowedOrigins.includes(returnOrigin)) {
      return res.status(403).json({ error: 'returnOrigin is not allowed by APP_ORIGIN' });
    }
    ...
    res.status(err.status || 500).json({ error: err.message || 'Failed to start authorization' });
  }
});
...
app.get('/api/oauth/:provider/callback', async (req, res) => {
  ...
  try {
    ...
    if (state.provider !== providerName) throw new Error('Provider does not match OAuth state');
    ...
    res.type('html').send(`<!doctype html><html lang="en"><meta charset="utf-8"><title>Authorization successful</title><body style="font-family:system-ui;padding:32px"><h2>✓ ${escapeHtml(p.label)} connected</h2><p>You can close this window and return to Xinliu.</p><script>try{window.opener&&window.opener.postMessage(${JSON.stringify({type:'iflow-oauth-success', provider:providerName, account})},${JSON.stringify(targetOrigin)});}catch(e){} setTimeout(()=>window.close(),500);<\/script></body></html>`);
  } catch (err) {
    console.error(providerName, 'oauth callback failed:', err);
    res.status(400).type('html').send(`<!doctype html><html lang="en"><meta charset="utf-8"><title>Authorization failed</title><body style="font-family:system-ui;padding:32px"><h2>Authorization failed</h2><p>${escapeHtml(err.message || 'Unknown error')}</p><p>Close this window and try again.</p></body></html>`);
  }
});

app.post('/api/oauth/:provider/disconnect', (req, res) => {
  const providerName = req.params.provider;
  if (!providers[providerName]) return res.status(404).json({ error: 'Unsupported provider' });
